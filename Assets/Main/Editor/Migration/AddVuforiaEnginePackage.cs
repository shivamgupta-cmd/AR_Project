...
public class AddVuforiaEnginePackage
{
    ...
    const string VUFORIA_VERSION = "10.29.6";
    ...
    {
        var migrationDirectory = Path.Combine(Directory.GetCurrentDirectory(), VUFORIA_TAR_FILE_DIR);
        if (!Directory.Exists(migrationDirectory))
            return new List<PackageDescription>();

        var tarFilePaths = Directory.GetFiles(migrationDirectory).Where(f => f.EndsWith(".tgz"));
